 ...
 * <ol>
 * <li>Translate polar radius and angle to tank drive thrusts (algorithm stolen from
 * http://robotics.stackexchange.com/questions/2011/ )</li>
...
public class PolarTankDrive implements IDrive {
  ...
  /**
   * If we are within this threshold (in degrees) from the desired heading, drive forward and turn
   * rather than rotate in place.
   */
  ...
  /**
   * Heading and thrust determined from single joystick. call setDirectionJoystick() before calling
   * this.
   */
  ...
  /**
   * Heading and thrust determined from different joysticks. call setDirectionJoystick() and
   * setThrustAxis() before calling this.
   */
  ...
  public void drive(double heading, double thrust) {
    final Pair<Double, Double> speed =
        tankSpeeds(thrust, heading, gyro.getAngle(), getRotateThreshold());
  ...
  public static Pair<Double, Double> tankSpeeds(final double power, final double desiredHeading,
      final double currentHeading, final double rotateThreshold) {
    final double thetaRelative = normalizeDegrees(desiredHeading - currentHeading);
    final double heading = Math.copySign(
        (Math.abs(thetaRelative) <= rotateThreshold) ? rotateThreshold : 180, thetaRelative);
  ...
  public static Pair<Double, Double> polarToThrust(final double theta, final double power) {
    ...
    return new Pair<>(Math.max(-1, Math.min(1, left)), Math.max(-1, Math.min(1, right)));
  }
