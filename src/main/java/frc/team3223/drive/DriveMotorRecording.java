...
public class DriveMotorRecording {
  ...
  public DriveMotorRecording(long tick, double frontLeft, double frontRight, double backLeft,
      double backRight) {
    this.tick = tick;
  ...
  public DriveMotorRecording interpolate(long now, DriveMotorRecording next) {
    if (this.tick != next.tick) {
      ...
      return new DriveMotorRecording(now, mfl * now + this.frontLeft - mfl * this.tick,
